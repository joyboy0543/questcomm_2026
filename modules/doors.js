(function(){
  ...
  function build(container){
      ...
      <div class="doors-hall">
        ${['alpha','beta','gamma'].map((k,i)=>`
          <div class="door-card ${state[k]?.open? 'open':''}" data-door="${k}">
            <div class="door-top">
              <div class="door-face" role="button" aria-label="Open mini-game">
    ...
    [...container.querySelectorAll('.door-card')].forEach((card, idx)=>{
      ...
      const correct = {alpha:'3', beta:'6', gamma:'7'}[key];
      ...
      function lockIfCorrect(){ if(input.value===correct){ input.readOnly=true; pill.style.display='inline-block'; state[key]={open:true, locked:true, digit:correct}; save(); radio(`Door ${['α','β','γ'][idx]} cleared. Digit recorded.`); checkAll(); } else { input.classList.add('shake'); setTimeout(()=>input.classList.remove('shake'),200); } }

      btn.addEventListener('click', lockIfCorrect);

      function mountGame(){ card.classList.add('open'); state[key] = state[key]||{open:true}; save();
        if(key==='alpha') window.initDoorMinesweep?.(host, ()=>{ showInput(); });
        if(key==='beta')  window.initDoorLockpick?.(host, ()=>{ showInput(); });
        if(key==='gamma') window.initDoorSnake?.(host, ()=>{ showInput(); });
      }
  ...
  function checkAll(){
    ...
    if(s.alpha?.locked && s.beta?.locked && s.gamma?.locked){
      ...
      // unlock next tab if exists
      const t = document.querySelector('.tab[data-tab="findings"], .tab[data-tab="next"]');
