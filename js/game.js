...
function restoreOpenCase(){
  const saved = localStorage.getItem('qc_open_case_title');
  if (saved) document.getElementById('current-case-title').textContent = saved;
  else {
...
let msgId = 1;
setInterval(()=> addRadioMessage(`Auto: Status update #${msgId++} — monitoring continues.`), PUBLISH_MS);
let scrollPaused = false; const SPEED=0.6;
(function loop(){
  if (!scrollPaused){
    ticker.scrollTop += SPEED;
    if (ticker.scrollTop + ticker.clientHeight >= ticker.scrollHeight) ticker.scrollTop = 0;
  }
...
ticker.addEventListener('mouseleave', ()=> scrollPaused = false);
