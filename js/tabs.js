...
(async function(){
  ...
  function refreshLocks(){ const ok2 = localStorage.getItem('qc_p2_ok')==='1'; const ok3 = localStorage.getItem('qc_p3_ok')==='1'; const ok4 = localStorage.getItem('qc_p4_ok')==='1'; const ok5 = localStorage.getItem('qc_p5_ok')==='1'; const final = localStorage.getItem('qc_p6_unlocked')==='1'; setDisabled(1, false); setDisabled(2, !ok2); setDisabled(3, !(ok2 && ok3)); setDisabled(4, !(ok2 && ok3 && ok4)); setDisabled(5, !(ok2 && ok3 && ok4 && ok5)); tabs[5].disabled = !(ok2 && ok3 && ok4 && ok5) && !final; }
  refreshLocks();

  window.QCPDUnlockNext = function(current){ const i = current-1; if (tabs[i+1]) { tabs[i+1].disabled=false; tabs[i+1].classList.add('unlocked'); setTimeout(()=> tabs[i+1].classList.remove('unlocked'), 1200); } refreshLocks(); QC.chips(); };
  window.QCPDUnlockFinal = function(){ tabs[5].disabled=false; tabs[5].classList.add('unlocked'); setTimeout(()=> tabs[5].classList.remove('unlocked'), 1200); localStorage.setItem('qc_p6_unlocked','1'); refreshLocks(); QC.chips(); };
})();
