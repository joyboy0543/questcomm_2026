...
window.QC = {
  saveProfile({department,name,password}){
    ...
    // Admin override via password keyword or URL param
    const url = new URL(window.location.href);
    if ((password||'').toLowerCase() === 'override-access' || url.searchParams.get('admin') === '1') {
  ...
  badge(el){ const {department,name}=this.loadProfile(); const div=document.getElementById(el); if(!div) return; div.innerHTML=''; const chip=document.createElement('div'); chip.className='chip'; chip.innerHTML = `<strong>${name||'Agent'}</strong> • ${department||'—'}`; if(localStorage.getItem('qc_admin')==='1'){ chip.innerHTML += ' • ADMIN'; } div.appendChild(chip); },
  chips(){ const el=document.getElementById('progress'); if(!el) return; el.innerHTML=''; const defs=[['Briefing','qc_p1_ok'],['Clearance','qc_p6_unlocked'],['Cipher','qc_p2_ok'],['Logic','qc_p3_ok'],['Network','qc_p4_ok'],['Pattern','qc_p5_ok']]; defs.forEach(([label,key])=>{ const chip=document.createElement('div'); chip.className='progress-chip'; const dot=document.createElement('span'); dot.className='dot'; chip.appendChild(dot); const txt=document.createElement('span'); txt.textContent=label; chip.appendChild(txt); if(localStorage.getItem(key)==='1'){ chip.classList.add('solved'); } el.appendChild(chip); }); },
