...
export function interrogate(rootSel = '#minigame'){
  ...
  root.querySelectorAll('.opts .btn').forEach(btn=>{
    btn.onclick = ()=>{ const ok = q.options[+btn.dataset.i].truth; btn.classList.toggle('correct', ok); alert(ok? 'Confession detected.' : 'Inconsistent statement.'); };
  });
}
export function fpScan(){
  const root = document.getElementById('fp-scan'); const btn = document.getElementById('scan-btn');
  if(!root || !btn) return;
