...
// Paths fixed to be relative for GitHub Project Pages
const cache = new Map();
const files = {
...
export function primeAudio(){
  Object.values(files).forEach(url => { const a=new Audio(); a.src=url; a.preload='auto'; a.muted=true; a.play().catch(()=>{}); });
}
...
export async function playRadio(lines=[], pauseMs=1200){
  for(const line of lines){ sfx('radio_beep',0.5); await toast(line); await wait(pauseMs); }
}
